...
def create_spectrograms_from_audio_dataset(
    ...
    return dataset_without_color_channel.map(
        map_func=lambda audio, label: (
            create_log_mel_sprectrogram(audio, sample_rate),
            label,
        ),
...
def extract_patches_from_audio_dataset(
    dataset: tf.data.Dataset, sample_rate=16000, patch_size=8
):
    dataset_without_color_channel = dataset.map(squeeze, tf.data.AUTOTUNE)
    ...
    return spectrogram_dataset.map(
        map_func=lambda mel_spec, label: (
            extract_patches_from_image(mel_spec, patch_size),
            label,
        ),
        num_parallel_calls=tf.data.AUTOTUNE,
    )
