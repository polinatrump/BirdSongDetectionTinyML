...
def lite_model_predict_dataset(
    tf_lite_model, x_data, y_true, input_data_uint8_type=False
):
    ...
    for i in range(len(x_data)):
        ...
        if input_data_uint8_type == True:
            interpreter.set_tensor(
                input_details[0]["index"], tf.cast(input_data, tf.uint8)
            )
        else:
...
def evaluate_prediction_with_threshold(
    y_true, y_pred_prob, model_format="tflite", optimal_threshold=None
):
    y_pred_optimal_threshold = (y_pred_prob >= optimal_threshold).astype(int)
    ...
    print(f"Precision: {precision * 100:.2f}%")
    print(f"F1-score: {f1score * 100:.2f}%")
    print(f"F2-score: {f2score * 100:.2f}%")
    ...
    return {
        ...
        "F2-score": round(f2score, 2),
    }


def get_f1_scores_of_bootstraping_partitions(
    model,
    x_data,
    y_true,
    model_format,
    input_data_uint8_type,
    n_bootstrap=100,
    n_chosen_samples=100,
):
    f1_scores = []
    ...
    for _ in range(n_bootstrap):
        ...
        if model_format == "keras":
            ...
            y_pred = tf_lite_model_predict(
                model, x_data[indices], input_data_uint8_type
            )
...
def get_f1_scores_of_non_overlapping_partitions(
    model, x_data, y_true, model_format, input_data_uint8_type, n_partitions=10
):
...
def predict_and_print_full_results(
    model, x_data, y_true, model_format, input_data_uint8_type=False
):
    # Predict
    if model_format == "keras":
    ...
    print(
        ...
        np.mean(non_overlap_patritions_f1_scores),
    )

    print(
        "\nGet 100 bootstrap samples from dataset with 100 samples each and get their mean F1-score"
    )
    bootstrap_patritions_f1_scores = get_f1_scores_of_bootstraping_partitions(
        model, x_data, y_true, model_format, input_data_uint8_type
    )
...
def find_decision_threshold(y_true, y_pred_prob, model_format):
    if model_format == "keras":
    ...
    for threshold in thresholds:
        ...
        # Update the best F1 score and threshold if the current F1 score is better
...
def evaluate_time_of_prediction(
    model,
    x_data,
    y_true_data,
    model_format,
    input_data_uint8_type=False,
...
def lite_model_from_file_predicts_dataset(
    model_path, x_data, input_data_uint8_type=False, threshold=None
):
    ...
    for i in range(len(x_data)):
        ...
        pred_label = np.argmax(pred_prob, axis=1)
        pred_labels.append(pred_label)
        y_pred_prob.append(pred_prob[0][1])

    return pred_labels, y_pred_prob
...
def create_spectrogram_features(audio, desired_length, sample_rate):
    ...
    log_mel_spectrogram_with_color_channel = tf.expand_dims(
        log_mel_spectrogram, axis=-1
    ).numpy()
...
def create_spectrogram_patches(audio, desired_length, sample_rate):
    ...
    log_mel_spectrogram_with_color_channel = tf.expand_dims(
        log_mel_spectrogram_with_color_channel, axis=0
    )
    patches = tf.image.extract_patches(
        log_mel_spectrogram_with_color_channel,
        sizes=[1, 8, 8, 1],
        strides=[1, 8, 8, 1],
        rates=[1, 1, 1, 1],
        padding="VALID",
...
def run_full_int_q_tflite_model(tflite_file, indices, x_data):
    ...
    for i, test_image_index in enumerate(indices):
        ...
        test_data_point = np.expand_dims(test_data_point, axis=0).astype(
            input_details["dtype"]
        )
...
def get_f1_scores_of_non_overlapping_partitions_full_int_q(
    tflite_file, x_data, y_true, n_partitions=10
):
    partition_size = len(x_data) // n_partitions
    partitions = []
...
def get_f1_scores_of_bootstraping_partitions_full_int_q(
    tflite_file, x_data, y_true, n_bootstrap=100, n_chosen_samples=100
):
    f1_scores = []
    n_samples = len(y_true)
    for _ in range(n_bootstrap):
