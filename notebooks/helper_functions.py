...
from sklearn.metrics import recall_score, accuracy_score, precision_score, f1_score, roc_curve, fbeta_score
import time
import random
...
def evaluate_prediction_with_threshold(y_true, y_pred_prob, model_format="tflite", optimal_threshold=None):
...
def get_f1_scores_of_bootstraping_partitions(model, x_data, y_true, model_format, input_data_uint8_type, n_bootstrap=100, n_chosen_samples=100):
    f1_scores = []
...
def predict_and_print_full_results(model, x_data, y_true, model_format, input_data_uint8_type=False):
    ...
    bootstrap_patritions_f1_scores = get_f1_scores_of_bootstraping_partitions(model, x_data, y_true, model_format, input_data_uint8_type)
...
def lite_model_from_file_predicts_dataset(model_path, x_data, input_data_uint8_type=False, threshold=None):
...
def get_f1_scores_of_bootstraping_partitions_full_int_q(tflite_file, x_data, y_true, n_bootstrap=100, n_chosen_samples=100):
    f1_scores = []
    n_samples = len(y_true)
    for _ in range(n_bootstrap):
        indices = np.random.choice(n_samples, size=n_chosen_samples, replace=True)
