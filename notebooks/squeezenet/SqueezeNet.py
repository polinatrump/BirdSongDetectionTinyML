...
def SqueezeNet(
    ...
    x = create_fire_module(
        x, int(16 * compression), name="fire3", use_bypass=use_bypass
    )
    ...
    x = create_fire_module(
        x, int(32 * compression), name="fire5", use_bypass=use_bypass
    )
    x = create_fire_module(x, int(48 * compression), name="fire6")
    x = create_fire_module(
        x, int(48 * compression), name="fire7", use_bypass=use_bypass
    )
    ...
    x = create_fire_module(
        x, int(64 * compression), name="fire9", use_bypass=use_bypass
    )
...
def output(x, nb_classes):
    x = Conv2D(
        nb_classes, (1, 1), strides=(1, 1), padding="valid", name="conv10"
    )(x)
...
def create_fire_module(x, nb_squeeze_filter, name, use_bypass=False):
    ...
    axis = get_axis()
    x_ret = Concatenate(axis=axis, name="%s_concatenate" % name)(
